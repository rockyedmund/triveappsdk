    ...
    function payToWallet(options) { //address, amount, message
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const openURL = `https://trvc.app/wallet/sendByQr?trivechain:${options.address}&dapp=${walletParams.dapp}&amount=${options.amount}`;

                resolve({c: 0, d: `https://trvc.app/wallet/sendByQr?trivechain:${options.address}&dapp=${walletParams.dapp}&amount=${options.amount}`});
